...
## Current implementation audit after trusted host assembly (2026-09-08 23:03 UTC)

The canonical worktree is `/Users/nv/repos/ubiquity/sentinel/.codex-worktrees/master-plan-gfa795549e5` on `codex/master-plan-gfa795549e5` at merge `42bfcd4b6051829f8c21ab4d2a02c3b417d188a2`. Accepted worker tip `44bbea7e03c9d71c20d3a2343646c7b67a3c1de3` is an ancestor. The merge adds the top-level trusted host assembly and provider adapters. The assembly now composes the existing GitHub, repair, and release host seams from explicit capabilities, preserves the release/repair capability split, and shares one injected clock and durable cooldown gate. Omitted model receipt and build receipt capabilities remain unavailable by default. No external capability or live side effect was used.

The registered post-merge host target `cc48556d9d6816f2605c36c352452867e2de5be694f7aa26c8a9330e9268c576/27be7a1a-318f-4676-97eb-52a5a5b051a9` failed only at two direct-entrypoint subprocess assertions because its definition omitted `--allow-run=deno`; the other `34` tests passed. A fresh primary command with the existing permission added passed the five host suites `36/36`. Static checks (`deno task check`, formatting, lint, and `git diff --check`) passed.

The registered full target `cc48556d9d6816f2605c36c352452867e2de5be694f7aa26c8a9330e9268c576/6815ea2b-fe40-430a-b4c5-baa3ce20eb69` ran at this exact revision and failed one existing scheduling-sensitive case after `679` passes: `tests/repair/loop-guard-boundary_test.ts` mode `stop-race` observed zero steering calls while the assertion expected one. The same file passed `15/15` in an independent run. This classifies the full receipt as non-clean under integrated load; the isolated pass is useful evidence but does not replace the failed full candidate.

The host boundary review found two follow-up risks. The top-level assembly does not validate `clock` or `githubCooldown` shapes before downstream construction. Nested shape checks and provider header enumeration access caller-owned properties directly, so an exotic getter can escape the promised static, non-echoing TypeError path. These are local hardening gaps to settle before activation; this audit made no source correction.

| Master-plan surface | Exact-head local evidence | Remaining divergence |
| --- | --- | --- |
| §4 / §9 scheduled entrypoints | Top-level host assembly and runner seams are present, and all five host suites pass with the required permissions. | Both workflows still invoke direct fail-closed tasks without trusted state, gateway/replay, GitHub, model/review, credentials, or build-receipt assembly. |
| §5 model admission and §7 review/merge | Durable rolling budgets, exact-head state machines, role separation, and local fake transports pass. | No credential-backed model session/receipt verifier, authenticated current-head review, aggregate publication, or accepted target PR exists. |
| §6 captured request to replay | Gateway capture/retention, sanitization, fixture binding, replay composition, and the target's local causal regression are green in separate local lanes. | No trusted production capture-to-loop host, owner-approved retention/key assembly, or live causal receipt exists. |
