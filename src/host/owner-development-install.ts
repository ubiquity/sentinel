...
/**
 * Exact revision recovering a closed-unmerged own pull request whose frozen
 * head predates the refreshed candidate; installed only after the closed-PR
 * generation 42 healthy proof.
 */
export const OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_REVISION =
  "a683d27e95a661e2cf7fba410f4a141257a659f3" as GitSha;
export const OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_GENERATION = 43;
/**
 * Exact revision assigning the trusted publication identity to the source
 * issue before any pull request, as the organization's anti-spam policy
 * requires; installed only after the closed-PR head generation 43 healthy
 * proof.
 */
...
/**
 * Exact revision that makes the repair lane actually close work: it writes a
 * bounded durable failure record on the exact intent (closed port kind plus a
 * closed detail, never raw upstream text), reconciles the record's own
 * merged/closed pull request BEFORE refreshing instead of re-arming an
 * `unavailable` wait forever, blocks a refresh that failed three consecutive
 * executions with the classified message, and demotes a record that spent the
 * per-record no-progress budget behind records that did advance inside its own
 * plan bucket; installed only after the review-phase PR retry generation 47
 * healthy proof.
 */
...
export function planOwnerDevelopmentInstall(
  ...
  if (
    ...
    if (healthy !== null) {
      return movePlan(
        "install",
        runtime,
        OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_REVISION,
        OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_GENERATION,
        healthy,
        "install the closed-PR head revision after the closed-PR healthy proof",
      );
    }
    return waiting(
  ...
  if (
    revision === OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_REVISION &&
    generation === OWNER_DEVELOPMENT_INSTALL_CLOSED_PR_HEAD_GENERATION
  ) {
    // An exact failed settlement of this pointer is candidate failure evidence
    // even when a healthy proof exists; the rollback still requires the
    // recorded healthy proof of the exact prior revision, so a missing or
    ...
    if (failed !== null) {
      ...
      if (prior === null) {
        return waiting(
          "the recorded closed-PR healthy proof for the rollback is unavailable",
        );
      }
      return movePlan(
        "rollback",
        ...
        runtime.generation + 1,
        prior,
        "roll back the failed closed-PR head candidate to its recorded prior",
      );
    }
    if (healthy !== null) {
      return movePlan(
        ...
        healthy,
        "install the assign-first revision after the closed-PR head healthy proof",
      );
    }
    return waiting(
      "the closed-PR head generation 43 healthy proof is not recorded",
    );
  }

  if (
    revision === OWNER_DEVELOPMENT_INSTALL_ASSIGN_FIRST_REVISION &&
    generation === OWNER_DEVELOPMENT_INSTALL_ASSIGN_FIRST_GENERATION
  ) {
    // An exact failed settlement of this pointer is candidate failure evidence
    // even when a healthy proof exists; the rollback still requires the
    // recorded healthy proof of the exact prior revision, so a missing or
    ...
    if (failed !== null) {
      ...
      if (prior === null) {
        return waiting(
          "the recorded closed-PR head healthy proof for the rollback is unavailable",
        );
      }
      return movePlan(
        "rollback",
        ...
        runtime.generation + 1,
        prior,
        "roll back the failed assign-first candidate to its recorded prior",
      );
    }
    if (healthy !== null) {
      return movePlan(
  ...
  if (
    ...
    if (healthy !== null) {
      return movePlan(
        ...
        healthy,
        "install the livelock-bound revision after the review-phase PR retry healthy proof",
      );
    }
    return waiting(
      "the review-phase PR retry generation 47 healthy proof is not recorded",
  ...
  if (
    revision === OWNER_DEVELOPMENT_INSTALL_LIVELOCK_BOUND_REVISION &&
    generation === OWNER_DEVELOPMENT_INSTALL_LIVELOCK_BOUND_GENERATION
  ) {
    // A failed candidate settles exactly once by rolling back to the recorded
    // prior — the proven review-phase PR retry revision — at a monotonic
    ...
    if (failed !== null) {
      ...
      if (prior === null) {
        return waiting(
          "the recorded review-phase PR retry healthy proof for the rollback is unavailable",
        );
      }
      return movePlan(
        "rollback",
        ...
        runtime.generation + 1,
        prior,
        "roll back the failed livelock-bound candidate to its recorded prior",
      );
    }
    if (healthy !== null) {
      return movePlan(
